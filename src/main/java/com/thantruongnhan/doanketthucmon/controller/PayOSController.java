...
public class PayOSController {
    ...
    public ResponseEntity<?> createPayment(@RequestBody CreatePaymentRequest req) {
        ...
        if (req.getItems() != null && !req.getItems().isEmpty()) {
            List<Map<String, Object>> cleanItems = req.getItems().stream()
                    .map(item -> {
                        Map<String, Object> cleanItem = new HashMap<>();
                        cleanItem.put("name", item.getName());
                        cleanItem.put("quantity", item.getQuantity());
                        cleanItem.put("price", item.getPrice());
                        return cleanItem;
                    })
                    .collect(Collectors.toList());

            payload.put("items", cleanItems);
        }

        try {
            Map<String, Object> response = payOSClient.createPaymentLink(payload);

            System.out.println(">>> PayOS Response Structure:");
